...
export function FlowList({
  ...
  const [searchResults, setSearchResults] = useState<MessageFlow[] | null>(
    null
  );
      ...
      if (filters.startTime) {
        ...
        if (filters.endpoint) {
          results = results.filter((f) =>
            f.messages.some((m) =>
              m.endpointName
                .toLowerCase()
                .includes(filters.endpoint!.toLowerCase())
            )
          );
        }
        ...
        if (filters.hasFailures !== undefined) {
          results = results.filter(
            (f) => f.hasFailures === filters.hasFailures
          );
        }
      } else {
        // Use search endpoint
      ...
      {displayFlows.length === 0 ? (
        <p className="text-gray-500 text-sm">
          {searchResults
            ...
            : "No flows yet. Send a message to see it here."}
        </p>
      ) : (
          ...
          {displayFlows.map((flow) => (
            ...
            >
              ...
              <div className="text-xs text-gray-400 mt-1 truncate">
                {[
                  ...new Set(flow.messages.map((m) => m.messageTypeShort)),
                ].join(" → ")}
              </div>
