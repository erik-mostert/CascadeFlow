...
import { TopologyGraph, type TopologyGraphRef } from './TopologyGraph';
...
export function TopologyView({ topology }: TopologyViewProps) {
    ...
    const handleExportPng = () => {
        ...
        if (dataUrl) {
            downloadPng(dataUrl, `topology-${new Date().toISOString().split('T')[0]}.png`);
        }
    };
        ...
        <div className="bg-gray-800 rounded-lg p-4 h-full flex flex-col">
            <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex items-center gap-4">
                    <ExportButton onExport={handleExportPng} label="Export PNG" icon="image" />
            ...
            <div className="flex-1 min-h-0">
                <TopologyGraph ref={graphRef} topology={topology} />
            </div>
            ...
            <div className="mt-4 flex-shrink-0">
                ...
                <div className="flex flex-wrap gap-2">
                    {Object.values(topology.endpoints).map(endpoint => (
                        <div
                            key={endpoint.name}
                            className={`rounded px-3 py-2 text-sm ${endpoint.failures > 0
