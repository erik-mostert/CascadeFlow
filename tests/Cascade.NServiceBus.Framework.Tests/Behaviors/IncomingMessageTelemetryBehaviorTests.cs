...
namespace Cascade.NServiceBus.Framework.Tests.Behaviors
{
    ...
    public class IncomingMessageTelemetryBehaviorTests
    {
        ...
        private IncomingMessageTelemetryBehavior _behavior;
        ...
        public async Task Invoke_CapturesProcessingDuration()
        {
            ...
            // Assert
            Assert.IsNotNull(capturedTelemetry);
            Assert.IsTrue(capturedTelemetry.ProcessingDuration.HasValue);
            Assert.IsTrue(capturedTelemetry.ProcessingDuration.Value.TotalMilliseconds >= 50);
        }
