...
namespace Cascade.NServiceBus.Framework.Tests.Dispatchers
{
    [TestClass]
    public class HttpTelemetryDispatcherTests
    {
        ...
        [TestMethod]
        public async Task Constructor_TrimsTrailingSlashFromCollectorUrl()
        {
            // Arrange
            var handler = new TestHttpMessageHandler();
            var httpClient = new HttpClient(handler);
            ...
            using (var dispatcher = new HttpTelemetryDispatcher(httpClient, options))
            {
                ...
                // Wait for background task to process (with timeout)
                var timeout = DateTime.UtcNow.AddSeconds(2);
                while (handler.LastRequestUri == null && DateTime.UtcNow < timeout)
                {
                    await Task.Delay(50);
                }

                // Assert - URL should not have double slashes
                Assert.IsNotNull(handler.LastRequestUri, "Request was not sent to the handler");
                Assert.IsTrue(handler.LastRequestUri.ToString().Contains("/api/telemetry"), "URL should contain /api/telemetry");
                Assert.IsFalse(handler.LastRequestUri.ToString().Contains("//api"), "URL should not contain //api (double slash)");
            }
        }
        ...
        public async Task DispatchAsync_DoesNotBlockCaller()
        {
            ...
            using (var dispatcher = new HttpTelemetryDispatcher(httpClient, options))
            {
                ...
                Assert.IsTrue(sw.ElapsedMilliseconds < 100, string.Format("DispatchAsync took {0}ms, expected < 100ms", sw.ElapsedMilliseconds));
            }
        }
